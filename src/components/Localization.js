...
const GlopeIcon = ({ width = 20, height = 20 }) => (
...
export const Localization = () => {
    ...
    const realseDate = new Date('2021-03-06')
    const timeDifference = new Date() - realseDate
    const number_of_days = Math.floor(timeDifference / (1000 * 60 * 60 * 24))
    const currentLangCode = Cookies.get('i18next') || 'ar'
    const currentSelectedLang = lang.find(l => l.code == currentLangCode)


    useEffect(() => {
        document.body.dir = currentSelectedLang.dir || 'ltr'
    }, [currentSelectedLang])
    return (
        <>
